...
def build_portfolio_from_vpp(
    ...
    portfolio = VPPPortfolio(
        ...
        peak_demand_cost_eur_per_kw=_cfg_number(vpp.config or {}, "peak_demand_cost_eur_per_kw", "peak_cost_eur_per_kw", default=0.0),
    )
    warnings: List[str] = []
        ...
        for device in site_devices:
            ...
            if kind == "battery":
                ...
                portfolio.add(BatteryAsset(asset_id, device.name, device.site_id, capacity_kwh=capacity,
                    ...
                    initial_soc=max(0.0, min(1.0, soc_pct / 100.0)), min_soc=_cfg_number(config, "min_soc", default=0.10),
                    max_soc=_cfg_number(config, "max_soc", default=0.95)))

            elif kind == "ev":
                portfolio.add(EVAsset(asset_id, device.name, device.site_id,
                    ...
                    arrival_hour=int(_cfg_number(config, "arrival_hour", default=0)), departure_hour=int(_cfg_number(config, "departure_hour", default=24))))

            elif kind == "industrial_load":
                portfolio.add(IndustrialLoadAsset(asset_id, device.name, device.site_id,
                    min_power_kw=_cfg_number(config, "min_power_kw", default=300.0),
                    max_power_kw=_cfg_number(config, "baseline_kw", "max_power_kw", default=450.0),
                    ...
                    baseline_kw=_cfg_number(config, "baseline_kw", default=450.0),
                ...
                portfolio.add(HeatPumpAsset(asset_id, device.name, device.site_id,
                    ...
                    max_power_kw=_cfg_number(config, "nominal_power_kw", "max_power_kw", default=20.0),
                    start_hour=int(_cfg_number(config, "start_hour", default=0)), end_hour=int(_cfg_number(config, "end_hour", default=horizon)),
                    baseline_power_kw=_cfg_number(config, "baseline_power_kw", "baseline_kw", default=8.0),
                    nominal_power_kw=_cfg_number(config, "nominal_power_kw", "max_power_kw", default=20.0),
                    initial_thermal_kwh=_cfg_number(config, "initial_thermal_kwh", default=50.0),
                    min_thermal_kwh=_cfg_number(config, "min_thermal_kwh", default=0.0), max_thermal_kwh=_cfg_number(config, "max_thermal_kwh", default=100.0),
                    thermal_gain_per_kwh=_cfg_number(config, "thermal_gain_per_kwh", default=1.0), thermal_loss_kwh=_cfg_number(config, "thermal_loss_kwh", default=0.0),
                    target_thermal_kwh=config.get("target_thermal_kwh"), operating_cost_eur_kwh=_cfg_number(config, "operating_cost_eur_kwh", default=0.0)))
                ...
                portfolio.add(FlexibleLoadAsset(asset_id, device.name, device.site_id,
                    ...
                    energy_required_kwh=_cfg_number(config, "energy_required_kwh", default=0.0), start_hour=int(_cfg_number(config, "start_hour", default=0)),
                    end_hour=int(_cfg_number(config, "end_hour", default=horizon)), curtailment_cost_eur_kwh=_cfg_number(config, "curtailment_cost_eur_kwh", default=0.0)))
