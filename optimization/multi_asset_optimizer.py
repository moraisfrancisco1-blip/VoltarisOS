...
class MultiAssetOptimizer:
    ...
    def optimize(self, portfolio: VPPPortfolio) -> MultiAssetOptimizationResult:
        started = time.time(); n = portfolio.horizon()
        if n <= 0: return MultiAssetOptimizationResult("error", [], 0.0, 0.0, 0.0)
        prices=self._series(portfolio.prices_eur_mwh,n); base_load=self._series(portfolio.base_load_kw,n)
        ...
        for a in batteries:
            ...
            for t in range(n):
                prev=a.initial_soc*a.capacity_kwh if t==0 else v["soc"][t-1]; prob+=v["soc"][t]==prev+v["charge"][t]*a.charge_efficiency-v["discharge"][t]/a.discharge_efficiency
