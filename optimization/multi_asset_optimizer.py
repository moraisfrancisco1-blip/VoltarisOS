...
class MultiAssetOptimizer:
    ...
    def optimize(self, portfolio: VPPPortfolio) -> MultiAssetOptimizationResult:
        ...
        for a in flex_loads:
            ...
            if isinstance(a,IndustrialLoadAsset):
                if a.recovery_kwh > 0 and a.max_recovery_kw > 0:
                    recovery=[LpVariable(f"{a.asset_id}_recovery_{t}",0,a.max_recovery_kw if a.start_hour<=t<min(a.end_hour,n) else 0.0) for t in range(n)]
                    for t in range(n):
                        prob += recovery[t] <= a.max_recovery_kw + flex_vars[a.asset_id][t]
                    prob += lpSum(recovery) == lpSum(-flex_vars[a.asset_id][t] for t in range(n))
                    prob += lpSum(recovery) <= a.recovery_kwh
                else:
                    prob += lpSum(flex_vars[a.asset_id]) == 0
        for a in heat_pumps:
            v=hp_vars[a.asset_id]
            for t in range(n):
                actual=a.baseline_power_kw+v["delta"][t]; prev=a.initial_thermal_kwh if t==0 else v["thermal"][t-1]
                prob += v["thermal"][t] == prev+actual*a.thermal_gain_per_kwh-a.thermal_loss_kwh
            if a.target_thermal_kwh is not None:
                target=min(max(a.end_hour-1,0),n-1); prob += v["thermal"][target] >= a.target_thermal_kwh
        ...
        for t in range(n):
            ...
            for a in flex_loads:
                actual=(a.baseline_kw+value(flex_vars[a.asset_id][t])) if isinstance(a,IndustrialLoadAsset) and a.start_hour<=t<min(a.end_hour,n) else (value(flex_vars[a.asset_id][t]) if not isinstance(a,IndustrialLoadAsset) else 0.0)
