...
The optimizer works with logical energy assets rather than physical devices.
A physical site may expose several devices that together form one asset.
"""
...
@dataclass
class EnergyAsset:
    """Base description shared by all controllable assets."""
...
class SolarAsset(EnergyAsset):
    """Non-dispatchable generation forecast."""
...
class BatteryAsset(EnergyAsset):
    """Electrical storage asset with SOC and power limits."""
...
class EVAsset(EnergyAsset):
    """EV or EV-fleet charging flexibility."""
...
class FlexibleLoadAsset(EnergyAsset):
    """Load that can move within a defined operating window."""
...
@dataclass
class VPPPortfolio:
    """Collection of assets optimized together."""
