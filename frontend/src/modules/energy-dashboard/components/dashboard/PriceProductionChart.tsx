...
interface PriceProductionChartProps {
  realPrices?: RealPrice[];
}

export function PriceProductionChart({ realPrices = [] }: PriceProductionChartProps) {
  const { lang } = useDashboardStore();
  const mockData = useMemo(() => generateHourlyData(), []);

  const data = useMemo(() => {
    if (realPrices && realPrices.length > 0) {
      return realPrices.map((p, i) => ({
        hour: p.hour,
        spotPrice: p.price / 1000,
        solar: mockData[i]?.solar ?? 0,
        profit: (p.price / 1000) * (mockData[i]?.solar ?? 0)
      }));
    }
    return mockData;
  }, [realPrices, mockData]);
  ...
  return (
    <GlassCard title={t('chart.priceVsProduction', lang)} accentColor="amber" explainText={t('explain.profitDrop', lang)} noPadding>
      <div style={{ padding: '8 8px 12px', height: 280 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 5, right: 15, left: -10, bottom: 0 }}>
            <defs>
              <linearGradient id="profitGrad2" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#f180981" stopOpacity={0.3} />
                <stop offset="100%" stopColor="#f10b081" stopOpacity={0} />
              </linearGradient>
            </defs>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
            <XAxis dataKey="hour" stroke="rgba(255,255,255,0.15)" tick={{ fill: '#6474b', fontSize: 10 }} tickLine={false} axisLine={false} interval={2} />
            <YAxis yAxisId="production" stroke="rgba(255,255,255,0.15)" tick={{ fill: '#6474b', fontSize: 10 }} tickLine={false} axisLine={false} unit=" kW" />
            <YAxis yAxisId="price" orientation="right" stroke="rgba(255,255,255,0.15)" tick={{ fill: '#6474b', fontSize: 10 }} tickLine={false} axisLine={false} unit="€" />
            ...
            <Line yAxisId="production" type="monotone" dataKey="profit" name={t('kpi.profit', lang)} stroke="#10b981" strokeWidth={2} dot={false} activeDot={{ r: 3 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </GlassCard>
