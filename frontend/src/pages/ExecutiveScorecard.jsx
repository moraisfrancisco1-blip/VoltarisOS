...
function HealthRing({ score, size = 140 }) {
  ...
  return (
    <svg width={size} height={size}>
      ...
      <text x={cx} y={cy + 14} textAnchor="middle" fill="var(--sub)" fontSize={10} fontWeight={600}>FLEET HEALTH</text>
    </svg>
  )
}
...
function RevenueGauge({ pct, size = 140 }) {
  ...
  return (
    <svg width={size} height={size}>
      ...
      <text x={cx} y={cy + 14} textAnchor="middle" fill="var(--sub)" fontSize={10} fontWeight={600}>VS TARGET</text>
    </svg>
  )
}
      ...
      <div style={card}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <ComposedChart data={FINANCIALS} margin={{ top: 5, right: 10, bottom: 0, left: -10 }}>
            ...
            <YAxis tick={{ fontSize: 10, fill: "var(--sub)" }} unit="k€" />
